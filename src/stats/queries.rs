...
impl StatsQuery {
    ...
    pub fn get_dashboard(&self, range: TimeRange, filter: &DashboardFilter) -> Result<DashboardSummary> {
        let conn = self.db.conn();
        let (current_bounds, previous_bounds) = self.time_bounds_with_prev(range);

        // Build WHERE clause based on filter
        let where_clause =
            self.build_where_clause(filter, current_bounds.start_ms, current_bounds.end_ms);
        let prev_where =
            self.build_where_clause(filter, previous_bounds.start_ms, previous_bounds.end_ms);
        let join_where = self.build_where_clause_for_alias(
            filter,
            current_bounds.start_ms,
            current_bounds.end_ms,
            "j",
        );
        let prev_join_where = self.build_where_clause_for_alias(
            filter,
            previous_bounds.start_ms,
            previous_bounds.end_ms,
            "j",
        );
    ...
    fn time_bounds_with_prev(&self, range: TimeRange) -> (TimeBounds, TimeBounds) {
        let now = Utc::now().timestamp_millis();
        match range.window_ms() {
            Some(window_ms) => {
                let current_start = now - window_ms;
                let current_end = now;
                let prev_start = current_start - window_ms;
                let prev_end = current_start;
                (
                    TimeBounds {
                        start_ms: Some(current_start),
                        end_ms: Some(current_end),
                    },
                    TimeBounds {
    ...
    fn build_where_clause_for_alias(
        ...
        let mut conditions = Vec::new();
        if let Some(start) = start_ms {
            conditions.push(format!("{alias}.created_at >= {}", start));
        }
        if let Some(end) = end_ms {
            conditions.push(format!("{alias}.created_at < {}", end));
        }
        if let Some(agent) = &filter.agent {
            conditions.push(format!(
                "{alias}.agent_type = '{}'",
